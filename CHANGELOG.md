...
- The generated protobuf and gRPC stub modules moved under
  `BroadwayCloudPubSub`, so `Google.Pubsub.V1.PubsubMessage` is now
  `BroadwayCloudPubSub.Google.Pubsub.V1.PubsubMessage`. The old top-level
  `Google.Pubsub.V1.*` names could collide with Pub/Sub stubs generated by the
  host application. Only the Elixir module names changed, so the wire format and
  RPC paths are the same and normal producer usage is unaffected. Add the prefix
  if you name these modules directly, which in practice means a custom
  `BroadwayCloudPubSub.Streaming.Client` or tests that build `ReceivedMessage`
  structs.
